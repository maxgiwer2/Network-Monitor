...
import { NmsTopology, INITIAL_NODES } from './components/NmsTopology';
...
function App() {
  ...
  const handleAddDevice = async (name: string, ip: string, details: string, sshUsername?: string, sshPassword?: string, deviceType?: string, uplinkId?: string) => {
    try {
      const response = await fetch('/api/devices', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, ip, details, sshUsername, sshPassword, deviceType, uplinkId })
      });
      if (!response.ok) {
      ...
      <section className="dashboard-grid">
        ...
        <div className="panel-card grid-col-8">
          ...
          <DeviceManager 
            ...
            availableUplinks={[...INITIAL_NODES, ...customDevices]}
            onAddDevice={handleAddDevice}
            onDeleteDevice={handleDeleteDevice}
